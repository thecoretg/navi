/* navi readability audit
 *
 * Paste into the browser console on any page built with navi, or run it
 ...
 *
 *   await naviAudit()                       // current theme + palette
 *   await naviAudit({ allPalettes: true })  // every palette x both themes
 */
(function () {
  ...
  window.naviAudit = async function ({ allPalettes = false } = {}) {
    ...
    console.log(`%cnavi audit — ${combos.length} combination(s), ${total} issue(s)`,
  ...
  console.log('naviAudit() ready — call it, or naviAudit({ allPalettes: true })');
})();
