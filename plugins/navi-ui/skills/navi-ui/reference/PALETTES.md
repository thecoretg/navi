...
they don't care. Record the answer in `.navi.json` and don't ask again.
...
1. Add one block like the above to the palettes section of `navi.css`.
...
- the `:root { --seed / --seed-dark / --tint / --c2…--c6 }` block at the top of
  `navi.css` (what renders before `data-palette` is set);
